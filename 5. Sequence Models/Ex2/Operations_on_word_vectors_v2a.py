...
def complete_analogy(word_a, word_b, word_c, word_to_vec_map):
    ...
    e_a, e_b, e_c = [word_to_vec_map[wrd] for wrd in [word_a, word_b, word_c]]
    ### END CODE HERE ###
...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = g * np.dot(e, g) / np.dot(g, g)
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    w1, w2 = pair
    e_w1, e_w2 = [word_to_vec_map[wrd] for wrd in pair]
    
    # Step 2: Compute the mean of e_w1 and e_w2 (≈ 1 line)
    mu = (e_w1 + e_w2) / 2

    # Step 3: Compute the projections of mu over the bias axis and the orthogonal axis (≈ 2 lines)
    mu_B = bias_axis * np.dot(mu, bias_axis) / np.dot(bias_axis, bias_axis)
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = bias_axis * np.dot(e_w1, bias_axis) / np.dot(bias_axis, bias_axis)
    e_w2B = bias_axis * np.dot(e_w2, bias_axis) / np.dot(bias_axis, bias_axis)
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    corrected_e_w1B = np.sqrt(np.abs(1 - np.dot(mu_orth, mu_orth))) * (e_w1B - mu_B) / np.sqrt(np.dot((e_w1 - mu_orth) - mu_B, (e_w1 - mu_orth) - mu_B))
    corrected_e_w2B = np.sqrt(np.abs(1 - np.dot(mu_orth, mu_orth))) * (e_w2B - mu_B) / np.sqrt(np.dot((e_w2 - mu_orth) - mu_B, (e_w2 - mu_orth) - mu_B))
